...
class Controller extends NamableComposite implements IBean
{
	...
	public function buildBean(array & $arrConfig,$sNamespace='*',\org\jecat\framework\bean\BeanFactory $aBeanFactory=null)
	{
		...
		// views --------------------
		if( !empty($arrConfig['view']) and is_string($arrConfig['view']) )
		{
			$arrConfig['view'] = array( 'template'=>$arrConfig['view'] ) ;
		}
		if( !empty($arrConfig['views']) )
		{
			$arrConfig['view']['views'] =& $arrConfig['views'] ;
		}
		...
		if( !empty($arrConfig['view']) )
		{
			...
			// 创建对象
			if( !$aView = $this->view(false) )
			{
				$aView = $aBeanFactory->createBean($arrConfig['view'],$sNamespace,false) ;
				$this->setView( $aView ) ;
			}
				
			$aView->buildBean($arrConfig['view'],$sNamespace) ;
